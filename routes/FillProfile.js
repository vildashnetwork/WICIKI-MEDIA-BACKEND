...
router.post("/update/privacy", async (req, res) => {
  try {
    ...
    const updatedUser = await User.findByIdAndUpdate(
      userId,
      {
        profilevissibility: ["public", "private"].includes(profilevissibility) ? profilevissibility : "public",
        twofactorenabled: normalizeBool(twofactorenabled, false),
        twofactormethod: ["email", "sms"].includes(twofactormethod) ? twofactormethod : "email",
        loginalerts: normalizeBool(loginalerts, true)
      },
      { new: true, runValidators: true, context: "query" }
    ).select("-password -__v");
    if (!updatedUser) return res.status(404).json({ message: "User not found" });
    ...
    console.error("POST /update/privacy error:", err);
    return res.status(500).json({ message: "Internal server error" });
  }
});
...
router.post("/update/notifications", async (req, res) => {
  try {
    ...
    const updatedUser = await User.findByIdAndUpdate(
      userId,
      {
        PushNotifications: normalizeBool(PushNotifications, true),
        emailsmsNotifications: normalizeBool(emailsmsNotifications, true),
        NotificationSounds: normalizeBool(NotificationSounds, true)
      },
      { new: true, runValidators: true, context: "query" }
    ).select("-password -__v");
    ...
    console.error("POST /update/notifications error:", err);
    return res.status(500).json({ message: "Internal server error" });
  }
});
...
router.post("/update/deactivate", async (req, res) => {
  try {
    const result = decodeTokenFromReq(req);
    if (!result.ok) return res.status(result.status).json({ message: result.message });
    const payload = result.payload;
    ...
    const updatedUser = await User.findByIdAndUpdate(
      userId,
      {
        Deactivate: normalizeBool(Deactivate, false)
      },
      { new: true, runValidators: true, context: "query" }
    ).select("-password -__v");
    if (!updatedUser) return res.status(404).json({ message: "User not found" });
    ...
    console.error("POST /update/deactivate error:", err);
    return res.status(500).json({ message: "Internal server error" });
  }
});
