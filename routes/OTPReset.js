...
// === Create TTL index for expiresAt so Mongo auto-deletes expired OTP docs ===
// This runs at startup. If the index already exists, Mongo will ignore it.
(async () => {
  try {
    await OTPmodel.collection.createIndex({ expiresAt: 1 }, { expireAfterSeconds: 0 });
    console.log("OTP TTL index ensured (expiresAt -> expireAfterSeconds: 0)");
  } catch (err) {
    console.warn("Could not create TTL index for OTP (maybe already exists):", err.message);
  }
})();
...
router.post('/send', async (req, res) => {
  ...
  try {
      ...
      htmlContent: `
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              <p>Hello <strong>${user.name || "there"}</strong>,</p>
              <p>We received a request to reset your password. Use the OTP below to continue. It’s valid for <strong>1 minute</strong>:</p>
              <div class="otp-box">${otp}</div>
              <p class="note">⚠️ If you did not request this, please ignore this email.</p>
            </div>
            <div class="footer">
    ...
    try {
      const result = await emailApi.sendTransacEmail(sendSmtpEmail);
      console.log(`OTP email sent to ${normalizedEmail} with messageId: ${result.messageId}`);
    } catch (emailError) {
      console.error('Error sending OTP email:', emailError);
      return res.status(500).json({ message: "Error sending OTP email" });
    }

    // return expiresAt so frontend can show exact countdown
    res.status(200).json({ message: "OTP sent to your email", success: true, expiresAt: expiresAt.toISOString() });
  } catch (error) {
    console.error("Error in request-reset-password:", error);
    res.status(500).json({ message: "Internal server error" });
  }
...
router.post('/validate', async (req, res) => {
  ...
  try {
    ...
    console.log("validate: found otpEntry:", otpEntry);

    if (!otpEntry) {
      return res.status(400).json({ message: "OTP not found or expired" });
    }

    // Defensive: ensure expiresAt is a Date
    let expiresAt = otpEntry.expiresAt;
    if (!(expiresAt instanceof Date)) {
      expiresAt = new Date(expiresAt);
    }

    // If expired: delete and return expired message
    if (expiresAt < new Date()) {
      try {
        await OTPmodel.deleteOne({ email: normalizedEmail });
      } catch (e) {
        console.warn("Failed to delete expired OTP:", e.message);
      }
      return res.status(400).json({ message: "OTP expired" });
    }
